...
def build_phase6() -> str:
    ...
    md += "### Dashboard Layout\n\n"
    md += "```mermaid\ngraph TD\n"
    md += "    A(\"Sidebar\") --> B(\"Node Selector<br/>Top-K from summary.json\")\n"
    md += "    B --> C(\"Node Stats<br/>Mean/Max Risk + Coordinates\")\n"
    md += "    B --> S(\"Map Slider<br/>High-risk intersections (5–500)\")\n"
    md += "    D(\"Section A\") --> E(\"PyDeck Map<br/>ScatterplotLayer + ArcLayer\")\n"
    md += "    E --> R(\"🔴 Selected node\")\n"
    md += "    E --> O(\"🟠 XAI top-K nodes\")\n"
    md += "    E --> Y(\"🟡 Extended nodes (slider)\")\n"
    ...
    md += "| Base Map Tiles | CartoDB Dark Matter (OSM) | Free tiles, no API key required |\n"
    ...
    md += "| Marker | Colour | Meaning |\n"
    md += "|--------|--------|--------|\n"
    md += "| 🔴 Red (large) | `[220, 30, 30]` | Currently selected intersection |\n"
    md += "| 🟠 Orange (medium) | `[255, 140, 0]` | Other XAI top-K nodes (summary.json) |\n"
    md += "| 🟡 Yellow (small) | `[255, 220, 0]` | Additional DR-ISI-ranked nodes (slider) |\n"
    md += "| Arc lines | Weight-mapped orange | GNNExplainer influence edges |\n"
    ...
    md += "### Data Loaders\n\n"
    md += "| Function | Source | Cache | Returns |\n"
    md += "|----------|--------|-------|---------|\n"
    md += "| `load_summary()` | `xai_artifacts/summary.json` | `@st.cache_data` | Top-K XAI nodes |\n"
    md += "| `load_node_positions()` | `processed_graph_data.pt` | `@st.cache_resource` | idx → (lat, lon) |\n"
    md += "| `load_all_node_risks()` | `processed_graph_data.pt` | `@st.cache_resource` | All non-zero DR-ISI nodes sorted by risk |\n"
    md += "| `load_subgraph(id)` | `xai_artifacts/subgraph_{id}.json` | `@st.cache_data` | GNNExplainer edges |\n"
    md += "| `load_temporal_profile(id)` | `xai_artifacts/temporal_profile_{id}.json` | `@st.cache_data` | Captum 24h attribution |\n"
    md += "| `load_local_shap(id)` | `xai_artifacts/local_shap_{id}.json` | `@st.cache_data` | Per-node SHAP values |\n"

    md += "\n### Input Artifacts\n\n"
    md += "| Artifact | XAI Source | Dashboard Section |\n"
    md += "|----------|-----------|-------------------|\n"
    md += "| `summary.json` | Model inference | Sidebar + Map |\n"
    md += "| `processed_graph_data.pt` | Phase 2 | Map extended nodes (slider) |\n"
